...
public:
    MultiSDWaveletTree(const wt_sequence_t & sequence, size_type arity);
...
template <class T>
MultiSDWaveletTree<T>::MultiSDWaveletTree(const wt_sequence_t & sequence, size_type arity) : 
    ARITY(arity),
...
template <class T>
m_sd_encoding_node_t makeMultiSDNode(const basic_string<T> & seq, size_type arity, size_type & seq_size)
{
    ...
    m_sd_encoding_node_t node;
    node.length = num_bits;
    node.sym_counts = counts;
    node.sdseq = m_sd_encoding_node_t::sdseq_ptr(
        new cds::BitSequenceSDArray(temp, num_bits));
    free(temp);
    ...
    node, size_type arity)
{
    ...
    //    node.sdseq->rank1((sym - 1) * length + length - 1);
    size_type rank_0 = (sym == 0)? 0 : node.sym_counts[sym - 1];
    // position that we actually want to query
    size_type real_pos = sym * length + pos;
    
    size_type rank_1 = node.sdseq->rank1(real_pos);
    
    return rank_1 - rank_0;
}

template <class T>
void MultiSDWaveletTree<T>::encodeNodeRecursive(const wt_sequence_t & sequence,
    size_type left, size_type right, size_type nodeIdx = 0)
{
    ...
    
    {
        ...
        encoding[nodeIdx] = makeMultiSDNode(mapped_sequence, ARITY, seq_size);
        seq_size += encoding[nodeIdx].sdseq->getSize() + 
            sizeof(m_sd_encoding_node_t);
    }
...
template <class T>
size_type MultiSDWaveletTree<T>::rankRecursive(T symbol, size_type pos,
