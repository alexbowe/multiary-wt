...
{
    classMapper = auto_ptr<Mapper>(new Mapper());
    blockMappers = vector< shared_ptr<Mapper> >();
}
    ...
    size_type & offset)
{
    ...
    if ( classNum >= class_table.size() )
        class_table.push_back(count_table_t());
    
    count_table_t & count_table = class_table[classNum];
